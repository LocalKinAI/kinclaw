...
- 用法: `memory action=save key=<dotted-path> value=<fact>`
       `memory action=recall query=<text>`
- 落地: SQLite k-v 表,**主动 recall 才会回到 prompt**

**判别(关键):** 这条信息 **下次别的 session 还需要的话** → `memory`。**只是这次任务用** → 不用记。
