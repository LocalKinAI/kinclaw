...
func buildInspirePrompt(candidateContent, sourceURL, skillRel string) string {
	...
	b.WriteString("  - command[0] MUST be a real binary in $PATH (osascript / curl / sh / python3 / installed CLI). NEVER a KinClaw internal skill name.\n")
	b.WriteString("  - args is a JSON-style array. Every {{var}} placeholder must be declared in schema.\n")
	b.WriteString("  - NO hardcoded screen coordinates (`click at {N,M}`) — forge gate v2 will reject.\n")
	b.WriteString("  - If the original references a binary that may not be installed by default (e.g. remindctl), say so in `caveats:`.\n")
	b.WriteString("  - Be honest. If your forge is a partial implementation, say which parts you punted in `caveats:` rather than faking the rest.\n")
	return b.String()
}
